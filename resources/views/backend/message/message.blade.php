<div id="messages">
  <a class="nav-link dropdown-toggle" href="#" id="messagesDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
      ...
      <!-- شمارنده - الرسائل -->
      @if(count(App\Http\Controllers\ShippingController::messageList())>5)
          <span data-count="5" class="badge badge-danger badge-counter">5+</span>
      @else 

  <span data-count="{{count(App\Http\Controllers\ShippingController::messageList())}}" class="badge badge-danger badge-counter">{{count(App\Http\Controllers\ShippingController::messageList())}}</span>
  ...
  <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="messagesDropdown">
      ...
      <div id="message-items">
          @foreach(App\Http\Controllers\ShippingController::messageList() as $message)
              <a class="dropdown-item d-flex align-items-center" href="{{route('message.show',$message->id)}}">
                  <div class="dropdown-list-image mr-3">
                      @if($message->photo)
                      <img class="rounded-circle" src="{{$message->photo}}" alt="البروفايل">
                      @else 
                      <img class="rounded-circle" src="{{asset('backend/img/avatar.png')}}" alt="تصویر عادي">
                      @endif
          ...
          <div>
              <div class="text-truncate">آیا من پسر خوبی هستم؟ دلیل اینکه می‌پرسم این است که کسی به من گفت که مردم این را به همه سگ‌ها می‌گویند، حتی اگر خوب نباشند...</div>
              <div class="small text-gray-500">مرغ سگ · 2w</div>
          </div>
          </a> --}}
      </div>
      <a class="dropdown-item text-center small text-gray-500" href="{{route('message.index')}}">بیشتر بخوانید</a>
  </div>
</div>
