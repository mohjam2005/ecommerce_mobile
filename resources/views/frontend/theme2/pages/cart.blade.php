					...
					<table class="table">
						<thead>
							<tr>
								...
								<th scope="col">القيمة واحد</th>
								<th scope="col">العدد</th>
								<th scope="col">القيمة نهایی</th>
							</tr>
						</thead>
						<tbody>
							<form id="cart-form" action="{{ route('update-cart') }}" method="POST">
								@csrf
								@if(App\Http\Controllers\ShippingController::getAllProductFromCart())
									@foreach(App\Http\Controllers\ShippingController::getAllProductFromCart() as $key => $cart)
										@php
										...
										<tr class="cart_item">
											...
											<td>
												<div class="cart_price">
													<del><span>{{ number_format($cart->product['price'], 2) }} <span>$</span></span></del>
													<ins><span>{{ number_format($cart->price, 2) }} <span>$</span></span></ins>
												</div>
											</td>
											...
											<td class="price_alltd" id="price_{{ $cart->id }}">{{ number_format($cart->amount, 2) }} <span>$</span></td>
										</tr>
									@endforeach
									<tr>
										<td colspan="5" class="text-right">
											<button class="btn btn-second-masai" type="submit">الدفع</button>
										</td>
									</tr>
								@else
									<tr>
				...
				<div class="row cart_details">
					<div class="cart-page-content col-xl-8 col-lg-7 col-md-7">
						<form action="{{ route('coupon-store') }}" method="POST">
							@csrf
							<input type="text" name="code" placeholder="کد نسبة التخفيض" class="input_second input_all width-200">
							<button type="submit" class="btn btn-main-masai">اعمال</button>
						</form>
						<div class="text_details">
							<p>ارسال رایگان برای الطلب‌های بالای 1 میلیون و 400 هزار $</p>
							<p>اضافة  کالا به سبد خرید به معنی رزرو آن نیست با توجه به محدودیت المخزن سبد خود را ثبت و خرید را تکمیل کنید.</p>
						</div>
					</div>
					<div class="cart-page-aside col-xl-4 col-lg-5 col-md-5 divider_details">
						<table class="table table_details">
							<tbody>
								...
								<tr>
									<td>القيمة نهایی:</td>
									<td class="total_price">{{ number_format(App\Http\Controllers\ShippingController::totalCartPrice(), 2) }} <span>$</span></td>
								</tr>
								<tr>
...
function updateCart(element, cartId, unitPrice) {
    ...
    document.getElementById('price_' + cartId).innerText = newPrice.toLocaleString('fa-IR', { minimumFractionDigits: 2 }) + ' $';

    // Update total price
    let totalPrice = 0;
    document.querySelectorAll('.price_alltd').forEach(function(priceElement) {
        totalPrice += parseFloat(priceElement.innerText.replace(/,/g, '').replace(' $', ''));
    });

    document.querySelectorAll('.total_price').forEach(function(totalElement) {
        totalElement.innerText = totalPrice.toLocaleString('fa-IR', { minimumFractionDigits: 2 }) + ' $';
    });
}
</script>
@endsection