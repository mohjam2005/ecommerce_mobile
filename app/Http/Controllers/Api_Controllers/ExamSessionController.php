...
class ExamSessionController extends Controller
{
    public function sessions(Request $request)
    {
         ...
         if(Auth::user()->hasRole('api'))
        {
            ...
            $user_id_examsession= exam_sessions::where('stud_id', $id_student)->where('exam_mode',$request->exam_mode)->where('exam_date',$exam_date_from_api_parse)->latest()->first();
            ...
            if($user_id_examsession==null){
                ...
                if($user_id_examsession->exam_status == 'انتهى'|| $user_id_examsession->exam_status == 'بدأ')
                    {
                        if($user_id_examsession->exam_status== 'انتهى' && $user_id_examsession->exam_class != $request->exam_class){
                            $student_session = exam_sessions::Create([
                            ...
                            $student_d = ["stud_id" => $id_student, "exam_id" => $student_session->exam_id];

                            return response()->json(['status'=>'success','data'=>$student_d]);

                        }
                        return response()->json(['status'=>'error','message'=>'لا يمكن اضافة امتحان بدأ أو انتهى في نفس اليوم ']);

                    }
                    else{
                        if($user_id_examsession->exam_class == $exam_class_from_api){
                            return response()->json(['status'=>'error','message'=>'لا يمكن حجز امتحان بنفس رخصة القيادة']);
                            ...
                            $user_id_examsession->exam_mode =$request->exam_mode;
